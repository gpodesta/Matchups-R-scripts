...
SST4.latband1.gam <- rep(NA, times=nrow(orig))

SST4.latband1.gam.res <- rep(NA, times=nrow(orig))
# ----------------------------------------------------------------------------------------
...
for (i in 1:12) {		# Begin looping through months of the year with matchups in them
  ...
  if (i == 1) {
    ...
    if (any(use.est)) {	# Check if there are any matchups within month and latitude band
      ...
      SST4.latband1.gam[which(use.calc)] <- (predvals - 0.17)
      SST4.latband1.gam.res[which(use.calc)] <- (predvals - 0.17) -
...
cat("# Algorithm version 6.3 FINAL\n",file=outfile, append=TRUE)
...
orig$SST4.latband1.gam <- ifelse(nite, SST4.latband1.gam, NA)
  
orig$SST4.latband1.gam.res <- ifelse(nite, SST4.latband1.gam.res, NA)
...
tt3 <- tapply(X=orig$SST4.latband1.res[orig$use.4.coeffs.SST4],
...
if (sensor == "TERRA") {
  ...
  rm(SST4.latband1, SST4.latband1.res, uu1,uu2,uu3); gc()
...
# --- Plots of SST4 and SST4 residuals vs. buoy

use <- orig$use.4.coeffs.SST == TRUE &
  (orig$SST4.latband1.res + 0.17) >= -1.0 &
  (orig$SST4.latband1.res + 0.17) <= 1.0 &
  orig$SST4.tree.class == "P-Good"

smoothScatter(orig$buoy.sst[use],
  orig$SST4.latband1[use],
  xlab="Buoy SST",
  ylab="Satellite SST4",
  main=paste0(sensor," - Final SST4"),
  nbin=60)
abline(-0.17, 1)

smoothScatter(orig$buoy.sst[use],
  orig$SST4.latband1.res[use],
  xlab="Buoy SST",
  ylab="Satellite SST4 Residuals",
  main=paste0(sensor," - Final SST4 residuals"),
  nbin=60)
abline(h=-0.17)
# ----------------------------------------------------------------------------------------
