...
SSTF.BTBAD <- abs(orig$cen.11000) < 1000 | abs(orig$cen.12000) < 1000
SST4.BTBAD <- abs(orig$cen.3959)  < 1000 | abs(orig$cen.3750) < 1000 |
  abs(orig$cen.3959) < 1000 | abs(orig$cen.4050) < 1000

SSTF.BTRANGE <- (orig$cen.11000 >= -4.0) & (orig$cen.11000 <= 37.0) |
...
SST4.BTRANGE <- (orig$cen.3959 >= -4.0) & (orig$cen.3959 <= 37.0) &
  (orig$cen.4050 >= -4.0) & (orig$cen.4050 <= 35.0)
...
SSTF.BTDIFF <- ((orig$cen.11000 - orig$cen.12000) >= 0) &
  ((orig$cen.11000 - orig$cen.12000) <= 3.6)

# --- The same, for SWIR channels

SST4.BTDIFF <- ((orig$cen.3959 - orig$cen.4050) >= 0) &
...
SSTF.SSTRANGE <- (orig$SST.latband1 >= -2.0) &
  ((orig$solz <= 90.0) & (orig$SST.latband1 <= 40.0)) |
  ((orig$solz > 90.0) & (orig$SST.latband1 <= 37.0))

SST4.SSTRANGE <- (orig$SST4.latband1 >= -2.0) & !is.na(orig$SST4.latband1) &
...
tt3 <- (orig$solz <= 90) | ((orig$solz > 90.0) & (orig$SST.latband1 - refsst) <= 5.0)
tt4 <- (orig$sat.lat > 40 | orig$sat.lat < -10)
tt5 <- (orig$sat.lon < -105 | orig$sat.lon > 105)
goodoutbox <- tt1 & tt2 & tt3 & tt4 & tt5

tt1 <- (refsst > -900 & !is.na(refsst))
tt2 <- (orig$SST.latband1  - refsst) >= -1.24
tt3 <- (orig$solz <= 90) | ((orig$solz > 90.0) & (orig$SST.latband1 - refsst) <= 5.0) # day or night and less than 5
tt4 <- (orig$sat.lat <= 40 & orig$sat.lat >= -10)
tt5 <- (orig$sat.lon >= -105 & orig$sat.lon <= 105)
goodinbox <- tt1 & tt2 & tt3 & tt4 & tt5
...
tt1 <- (refsst > -900 & !is.na(refsst))
tt2 <- (orig$SST4.latband1 - refsst) >= -1.24
...
SSTF.BTNONUNIF <- ((orig$max.11000 - orig$min.11000) <= 0.7) &
  ((orig$max.12000 - orig$min.12000) <= 0.7)

# --- Range of differences in BT22 and BT23 inside window

SST4.BTNONUNIF <- ((orig$max.3959 - orig$min.3959) <= 0.7) &
...
SSTF.BTVNONUNIF <- ((orig$max.11000 - orig$min.11000) <= 1.2) &
  ((orig$max.12000 - orig$min.12000) <= 1.2)

# --- Range of differences in BT22 and BT23 inside window

SST4.BTVNONUNIF <- ((orig$max.3959 - orig$min.3959) <= 1.2) &
...
SSTF.BT4REFDIFF <- (orig$cen.39.40.ref.new >= -1.1) &  (orig$cen.39.40.ref.new <= 10.0)
...
SSTF.SSTREFVDIFF <- (refsst  > -900 & !is.na(refsst)) 
((orig$SST.latband1  - refsst) >= -5.0) &
  ((orig$solz <= 90.0) |((orig$solz > 90.0) & orig$SST.latband1 - refsst <= 5.0))

SST4.SSTREFVDIFF <- (refsst > -900 & !is.na(refsst) & !is.na(orig$SST4.latband1)) &
...
RED.UNIF.SET.OLD.TRUE <- (orig$qsst == 2 & SSTF.BTNONUNIF) | (orig$qsst == 3 &
  orig$SST.tree.class == "P-Good" & SSTF.BTRANGE & SSTF.SSTRANGE & 
  SSTF.BTVNONUNIF & SSTF.SSTREFVDIFF & SSTF.VHISENZ & SSTF.BTNONUNIF & orig$glint < 0.005) # if reduniformity is bad if TRUE
  
SSTF.REDNONUNIF <- (orig$solz > 90.0) | ((orig$solz <= 90.0) & !RED.UNIF.SET.OLD.TRUE) # night or day and old reduniformity is NOT bad so all is well
...
SSTF.DEMOTED.REDHIGH.TRUE <- (orig$SST.latband1  - refsst) <= -1 &
  ((orig$qsst == 1 & orig$SST.tree.class == "P-Good" &
   ...
   (orig$qsst == 2 & orig$SST.tree.class == "P-Good" &  SSTF.BTRANGE & SSTF.SSTRANGE & 
   SSTF.REDNONUNIF & SSTF.BTVNONUNIF & SSTF.SSTREFVDIFF &
      SSTF.SSTREFDIFF & SSTF.VHISENZ &  orig$glint < 0.005) |
   (orig$qsst == 3 & orig$SST.tree.class == "P-Good" & SSTF.BTRANGE & SSTF.SSTRANGE & 
   SSTF.BTVNONUNIF & SSTF.SSTREFVDIFF & SSTF.VHISENZ & orig$glint < 0.005)) # if true pixel QL will need to be demoted
...
tt3 <- SST.quality.tests[, "qsst.new"] == 4 &
  SST.quality.tests[,"day.or.nite"] =="Day" &
  (SST.quality.tests[,"btbad"] &
  	orig$img.SST.tree.class == "P-Bad" | ### INCONSISTENCY
  	!SST.quality.tests[,"vhisenz"] |
  	!SST.quality.tests[,"btrange"] |
  	!SST.quality.tests[,"sstrange"] |
  	!SST.quality.tests[,"btvnonunif"] |
...
# see new hogh rht proxy code by KAK may 2013 DEMOTED.REDHIGH.TRUE near line 308 

tt5 <- SST.quality.tests[, "demoted.redhigh.true"] & SST.quality.tests[, "qsst.new"] < 3
...
# If SST4 pases the clud tree is defined by orig$SST4.tree.class == P-Good
...
tt3 <-  SST4.quality.tests[, "qsst4.new"] == 4 &
  ...
  (SST4.quality.tests[,"btbad"] &
  	...
  	!SST4.quality.tests[,"sstrange"] |
  	!SST4.quality.tests[,"bt4refdiff"] |
  	orig$img.SST4.tree.class == "P-Bad" |
  	!SST4.quality.tests[,"btvnonunif"] |
  	!SST.quality.tests[,"vhisenz"] |
  	!SST4.quality.tests[,"sstrefvdiff"]
  	)
...
# --- All SST4 quality levels for daytime matchups have the value of 3 as long as the BT is less than 1000

tt4 <- SST4.quality.tests[,"day.or.nite"] == "Day" &
	!SST.quality.tests[,"btbad"] 

SST4.quality.tests[tt4, "qsst4.new"] <- 4
...
addmargins(xtabs(~ SST4.quality.tests[, "qsst4.new"]))
round(prop.table(xtabs(~ SST4.quality.tests[, "qsst4.new"])),4) * 100
# ----------------------------------------------------------------------------------------
...
barplot(tt1, names=names(tt1),
  ...
  main=paste(sensor,"- SST Quality Levels"),
  col="steelblue")
box()
...
barplot(tt1, names=names(tt1),
  ...
  ylab="Proportion of Matchups",
  main=paste(sensor,"- SST4 Quality Levels"),
  col="steelblue")
box()
...
# ----------------------------------------------------------------------------------------
# --- Calculate statistics of SST4 residuals for each quality level ----
...
tt10 <- round(tapply(orig3$SST4.latband1.res, INDEX=orig3$qsst.new, FUN=length, simplify=TRUE),3)
...
histogram(~ orig3$SST.latband1.res | tt1,
  ...
  main=paste(sensor,"SST - Residuals by SST quality level"),
...
histogram(~ orig3$SST4.latband1.res | tt1,
  ...
  main=paste(sensor, "SST4 - Residuals by SST4 quality level"),
