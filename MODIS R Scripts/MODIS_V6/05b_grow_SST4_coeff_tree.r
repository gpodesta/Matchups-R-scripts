...
sst4.res.use <- orig$SST4.latband1.prelim2.res[nite] + 0.17

# --- Define a trheshold to consider residuals bad or good

resid.threshold <- 0.50  	# Threshold to consider residuals good or bad
...
hist(sst4.res.use); box()

hist(sst4.res.use[res.class == "Good"])

library(Hmisc)
Ecdf(sst4.res.use, what="F", xlim=c(-1.5, 1.5),
     xlab="SST4 latband1 residual", add=FALSE)
abline(v=c(-0.5, 0.5))
# ----------------------------------------------------------------------------------------
...
rpart.plot(x=nite.tree, type=1, extra=1,
  ...
  main=paste(sensor, "- SST4 Night tree"))
...
pred.vals.nite <- data.frame(SST4.actual.class=actual.class,
  SST4.pred.class=pred.class,
  SST4.prob.bad=prob.bad,
  SST4.prob.good=prob.good)
...
predicted.categories <- as.data.frame(matrix(data=NA,
  ...
  dimnames=list(NULL, c("SST4.actual.class","SST4.tree.class",
  "SST4.tree.prob.BAD", "SST4.tree.prob.GOOD"))))  	# Store predicted categories here

# --- Store results in object "predicted.categories"

predicted.categories[nite, ] <- pred.vals.nite

table(predicted.categories$SST4.actual.class, useNA="always")
table(predicted.categories$SST4.tree.class, useNA="always")

addmargins(xtabs(~ SST4.tree.class + SST4.actual.class, data=predicted.categories))   # add row/col summary (default is sum)
addmargins(prop.table(xtabs(~ SST4.tree.class + SST4.actual.class, data=predicted.categories)))   # show counts as proportions of total
# ----------------------------------------------------------------------------------------

# ---------------------------------------------------------------------------------------#
# --- Histogram of the probability of SST4 matchups being good Nite + Day

hist(predicted.categories[,"SST4.tree.prob.GOOD"],
     breaks=seq(0,1,0.1),
     xlim=c(0,1),
     xlab="Probability of GOOD",
     main=paste(sensor,"SST4 Night - P(good)"),
...
orig$SST4.actual.class <- ordered(orig$SST4.actual.class,
  levels=c(1, 2), labels=c("Bad", "Good"))

orig$SST4.tree.class <- ordered(orig$SST4.tree.class,
...
# --------------------------------------------------------------------------------------------------
# --- Exeriment with RandomForest classifier
