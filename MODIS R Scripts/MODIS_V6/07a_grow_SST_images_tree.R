...
# --- Grow a cloud tree to be used on SST imagery
...
# ---------------------------------------------------------------------------------------#
# ----------------------------------------------------------------------------------------
# --- PART 1. Growing a NIGHTTIME tree to be used on IMAGES ----
# ----------------------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------------------#
# --- Define "good" and "bad" SST residuals ---- 
# --- Note we grow the image tree using final SST residuals
...
hist(sst.res.use,
  xlab= "SST final residuals",
  main="SST latband1 FINAL residuals")
box()

library(Hmisc)
Ecdf(sst.res.use, what="F", xlim=c(-2.5, 2.5),
  lwd=2, col="tomato",
  xlab="SST latband1 FINAL residuals", add=FALSE)
abline(v=c(-1.5, 1.5))
# ----------------------------------------------------------------------------------------
...
rpart.plot(x=img.nite.tree, type=1, extra=2,
  under=TRUE,uniform=TRUE, digits=6,
  main=paste(sensor, "-", geophys.var, "- IMAGERY Night tree"))

library(maptree)
draw.tree(img.nite.tree)

dput(img.nite.tree,
  file = "D:/Matchups/MODIS/results/version_6b/Aqua/collection_6/trees/aqua_night_img_SST_tree_dput.txt")
...
title(paste(sensor, " - ", geophys.var,"IMAGERY Night tree"))
...
pred.vals.nite <- data.frame(img.SST.actual.class=actual.class,
  img.SST.pred.class=pred.class,
  img.SST.prob.bad=prob.bad,
  img.SST.prob.good=prob.good)
...
predicted.categories <- as.data.frame(matrix(data=NA,
  ...
  dimnames=list(NULL, c("img.SST.actual.class","img.SST.tree.class",
  "img.SST.tree.prob.BAD", "img.SST.tree.prob.GOOD"))))		# Store predicted categories here

# --- Store results in object "predicted.categories"

predicted.categories[nite, ] <- pred.vals.nite

table(predicted.categories$img.SST.actual.class, useNA="always")

table(predicted.categories$img.SST.tree.class, useNA="always")

table(predicted.categories$img.SST.actual.class,
  predicted.categories$img.SST.tree.class, useNA="always")

# --- Plot probabilities of being good
save.plots <- TRUE

if (save.plots) {
  #wmf.graph(paste(graph.outdir,sensor,"-img.nite.tree.barplot.wmf", sep=""))
  #pdf.graph(paste(graph.outdir,sensor,"-img.nite.tree.barplot.pdf", sep=""))
  
  png(filename=paste(graph.outdir,sensor,"-img.nite.tree.barplot.wmf", sep=""),
      width = 480, height = 480, units = "px")
  
  tt1 <- table(pred.vals.nite[,"img.SST.prob.good"])
  tt2 <- as.character(round(as.numeric(names(tt1)),3))
  barplot(tt1, names=tt2,
    xlab="Probability of being GOOD",
    ylab="Number of matchups",
    main=paste(sensor, "IMAGE Nighttime Tree"),
    col="goldenrod", las=1)  
  box()
...
hist(pred.vals.nite[,"img.SST.prob.good"],
  breaks=seq(0,1,0.1),
  xlim=c(0,1),
  xlab="Probability of GOOD",
  main=paste(sensor,"IMAGE Night tree - P(good)"),
...
rm(loss.matrix,confusion.matrix.results, lat,latband, nite, img.nite.tree)
...
#  solz <- orig$solz[day]   # This variable is NOT used for IMAGE daytime tree
...
delta.sst <- orig$buoy.sst[day] - orig$ref.type.1.SST[day]
# ----------------------------------------------------------------------------------------

# ---------------------------------------------------------------------------------------#
# --- Define "good" residuals as those with abs(resid) < threshold deg C ----

sst.res.use <- orig$SST.latband1.res[day] + 0.17

# --- Create a factor for the SST residual quality ("bad" or "good")

res.class <- ifelse((sst.res.use < -1.5) | (sst.res.use > 3.5),
  "Bad", "Good")
...
day.input.data <- data.frame(res.class, sst.res.use,
  ...
  in.glint, satz, latband, delta.sst)
...
rpart.plot(x=img.day.tree, type=1, extra=1,
  under=TRUE,uniform=TRUE, digits=6,
  main=paste(sensor, "-", geophys.var, "- Daytime tree"))


dput(img.day.tree,
  file = "D:/Matchups/MODIS/results/version_6b/Terra/collection_6/trees/terra_day_img_SST_tree_dput.txt")
...
plot(img.day.tree, uniform=TRUE)
text(img.day.tree, cex=0.6)
title(paste(sensor, " - ", geophys.var,"Daytime tree"))

draw.tree(img.day.tree)
...
pred.test <- predict(img.day.tree, newdata=newdata, type="matrix")
...
pred.test <- as.data.frame(predict(img.day.tree, newdata=day.input.data, type="matrix"))
...
pred.vals.day <- data.frame(img.SST.actual.class=actual.class,
  img.SST.pred.class=pred.class,
  img.SST.prob.bad=prob.bad,
  img.SST.prob.good=prob.good)
...
if (save.plots) {
  #wmf.graph(paste(graph.outdir,sensor,"-img.day.tree.barplot.wmf", sep=""))
  #pdf.graph(paste(graph.outdir,sensor,"-img.day.tree.barplot.pdf", sep=""))
  
  png(filename=paste(graph.outdir,sensor,"-img.day.tree.barplot.wmf", sep=""),
      width = 480, height = 480, units = "px")
  
  tt1 <- table(pred.vals.day[,"img.SST.prob.good"])
  tt2 <- as.character(round(as.numeric(names(tt1)),3))
  barplot(tt1, names=tt2,
    xlab="Probability of being GOOD",
    ylab="Number of matchups",
    main=paste(sensor, "IMAGE Daytime Tree"),
    col="goldenrod", las=2)
  box()
...
hist(pred.vals.day[,"img.SST.prob.good"],
  breaks=seq(0,1,0.1),
  xlim=c(0,1),
  xlab="Probability of GOOD",
  main=paste(sensor,"IMAGE Daytime tree - P(good)"),
  col="tomato", freq=FALSE)
box()
# ----------------------------------------------------------------------------------------
...
hist(predicted.categories[,"img.SST.tree.prob.GOOD"],
     breaks=seq(0,1,0.1),
     xlim=c(0,1),
     xlab="Probability of GOOD",
...
orig$img.SST.actual.class <- ordered(orig$img.SST.actual.class,
  levels=c(1, 2), labels=c("Bad", "Good"))

orig$img.SST.tree.class <- ordered(orig$img.SST.tree.class,
  levels=c(1, 2), labels=c("P-Bad", "P-Good"))

addmargins(xtabs(~ img.SST.tree.class + img.SST.actual.class, data=orig))   # add row/col summary (default is sum)
