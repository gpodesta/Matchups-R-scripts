...
# --- NOTE: We no longer calculate statistics
...
uu0 <- (orig$cen.11000 - orig$cen.12000)
...
if (sst.algo == "latband1") {
	...
	SST.res <- orig$SST.latband1.res + skin.offset		# SST latband1 residuals
	qsst <- orig$qsst.new												      # SST latband1 quality levels
} else if (sst.algo == "V5") {
	cat("Looking at residuals for",sst.algo,"SSTs...\n")
	SST.res <- orig$SST.V5.res + skin.offset					# SST V5 residuals
	qsst <- orig$PF5.qsst 														# SST V5 quality levels
}
