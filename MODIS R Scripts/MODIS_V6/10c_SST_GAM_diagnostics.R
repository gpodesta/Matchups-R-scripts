...
# ---------------------------------------------------------------------------------------#
# --- Plot SST residuals as a fucntion of satellite zenith angle
...
smoothScatter(orig$satz[use], orig$SST.latband1.res[use],
  ...
  ylab="SST Residuals",
  main=paste("MODIS ",sensor,"\n SST Latband1 residuals - realistic selection"))

uu1 <- loess(orig$SST.latband1.res[use] ~ orig$satz[use], span=0.4)
...
hbin <- hexbin(orig$satz[use], orig$SST.latband1.gam.res[use], xbins=60)

plot(hbin, mincnt=100)
...
if (plot.diagnostics) {
  ...
  levelplot(res.mean ~ mon * lat,
    ...
    pretty=TRUE, at=bb2,          
    xlab="Month", ylab="Latitude band",
    main= paste(sensor, "Mean of SST latband1 residuals"),
    ylim=c(0.5, 6.5),
    col.regions=bb5,
  ...
  levelplot(res.median ~ mon * lat,
    ...
    pretty=TRUE, at=bb2,          
    xlab="Month", ylab="Latitude band",
    main= paste(sensor, "Median of SST latband1 residuals"),
    ylim=c(0.5, 6.5),
    col.regions=bb5,
    scales = list(x= list(cex=0.7, rot=0, at=seq(1,length(seq.months),12),
      labels=seq.months[seq(1,length(seq.months),12)]),
...
if (plot.diagnostics) {
  ...
  levelplot(res.sd ~ mon * lat,
    ...
    pretty=TRUE, at=bb2,          
    xlab="Month", ylab="Latitude band",
    main= paste(sensor, "Std. Dev. of SST latband1 residuals"),
    ylim=c(0.5, 6.5),
    col.regions=bb5,
    scales = list(x= list(cex=0.7, rot=0, at=seq(1,length(seq.months),12),
      labels=seq.months[seq(1,length(seq.months),12)]),
  ...
  levelplot(res.mad ~ mon * lat,
    ...
    pretty=TRUE, at=bb2,          
    xlab="Month", ylab="Latitude band",
    main= paste(sensor, "MAD of SST latband1 residuals"),
    ylim=c(0.5, 6.5),
    col.regions=bb5,
    scales = list(x= list(cex=0.7, rot=0, at=seq(1,length(seq.months),12),
      labels=seq.months[seq(1,length(seq.months),12)]),
