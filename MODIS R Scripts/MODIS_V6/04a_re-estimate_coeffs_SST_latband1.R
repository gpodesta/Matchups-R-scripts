...
SST.latband1.gam.prelim2 <- rep(NA, times=nrow(orig))

SST.latband1.gam.prelim2.res <- rep(NA, times=nrow(orig))
# ----------------------------------------------------------------------------------------
...
for (i in 1:12) {		# Begin looping through months of the year with matchups in them
  ...
  if (i == 1) {
    ...
    use <- orig$use.4.coeffs.SST == TRUE &
      (orig$SST.latband1.prelim.res + 0.17) >= -1.5 &
	    (orig$SST.latband1.prelim.res + 0.17) <= 1.5
    ...
    # Check if there are any matchups within month and latitude band
      ...
      SST.latband1.gam.prelim2[indices.use.calc] <- (predvals - 0.17)
      SST.latband1.gam.prelim2.res[indices.use.calc] <- (predvals - 0.17) -
...
orig$SST.latband1.gam.prelim2 <- SST.latband1.gam.prelim2

orig$SST.latband1.gam.prelim2.res <- SST.latband1.gam.prelim2.res
# ----------------------------------------------------------------------------------------
...
SST.latband1.prelim2 <- coef1 + (coef2 * x1) + (coef3 * x5) + (coef4 * x6) +
  (coef5 * x7) + (coef6 * x4) + (coef7 * (x4 ^ 2))

smoothScatter(orig$buoy.sst[orig$use.4.coeffs.SST],
  SST.latband1.prelim2[orig$use.4.coeffs.SST],
  xlab="Buoy SST",
  ylab="Satellite SST",
  main=paste0(sensor," - Re-estimated SST"),
  nbin=60)
abline(-0.17,1)
...
SST.latband1.prelim2.res <- SST.latband1.prelim2 - aa2$buoy.sst		# SST residuals

smoothScatter(orig$buoy.sst[orig$use.4.coeffs.SST],
  SST.latband1.prelim2.res[orig$use.4.coeffs.SST],
  xlab="Buoy SST",
  ylab="Satellite SST Resdiuals",
  main=paste0(sensor," - Re-estimated SST residuals"),
  nbin=60)
abline(h=-0.17)

# --- Replace previously-estimated latband SST and residuals in data frame "aa2".

aa2$SST.latband1.prelim2 <- SST.latband1.prelim2 						# Replace preliminary SST values
aa2$SST.latband1.prelim2.res <- SST.latband1.prelim2.res 		# Replace preliminary residual values
...
rm(SST.latband1.prelim2, SST.latband1.prelim2.res); gc()
# ----------------------------------------------------------------------------------------
...
tt0 <- orig$SST.latband1.prelim2.res
