# ---------------------------------------------------------------------------------------#
# --- Script to estimate PRELIMINARY "latband1" SST algorithm coefficients
# --- for MODIS matchups.
# --- In the "latband1" algorithm, zonal bands are statically defined by latitude.
# ----------------------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------------------#
# --- Reserve some of the data to test SST algorithm performance ----
...
orig$train.sst[ind.training] <- "Train"		# Training set

table(orig$train.sst, useNA="ifany")

rm(orig.use, n.orig.use, n.training, ind.training, prop.training); gc()
# ----------------------------------------------------------------------------------------
...
SST.latband1.prelim.gam <- rep(NA, times=nrow(orig))

SST.latband1.prelim.gam.res <- rep(NA, times=nrow(orig))

# --- Also create a dataframe to store GAM coefficients

gam.coeffs <- matrix(NA, ncol=7, nrow=nrow(orig))
colnames(gam.coeffs) <- c("mon","latband","intcp","ch11000","btd","satz","mirror")
gam.coeffs <- as.data.frame(gam.coeffs)
# ----------------------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------------------#
# --- Iterate to estimate SST latband1 algorithm coefficients ----
...
for (i in 1:12) {		# Begin looping through months of the year with matchups in them
  ...
  if (i == 1) {
    ...
    # Check if there are any training matchups within month window and latitude band
      ...
      xreg <- lm(bsst ~ ch11000 + btd.sst + secterm,
        data=reg.input.2,
        weights = xwtf,
      	subset=(xwtf > weight.threshold), na.action = na.omit)
      
      coef1[i, j] <- xreg$coef[1]			# intercept coeff
      coef2[i, j] <- xreg$coef[2]     # ch11000 coeff
      coef3[i, j] <- xreg$coef[3]     # temp difference coeff
      coef4[i, j] <- xreg$coef[4]     # secant term coeff
      ...
      xreg.gam <- gam(bsst ~ ch11000 + s(BTdiff) + s(satz),
        data = reg.input.2,
        weights = xwtf,
        method = "REML",
      ...
      SST.latband1.prelim.gam[which(use.calc)] <- predvals - 0.17
      SST.latband1.prelim.gam.res[which(use.calc)] <- predvals - 0.17 -
...
# ---------------------------------------------------------------------------------------#
# --- Save GAM SST estimates in object "orig" ----

orig$SST.latband1.prelim.gam <- SST.latband1.prelim.gam 
  
orig$SST.latband1.prelim.gam.res <- SST.latband1.prelim.gam - orig$buoy.sst
# ----------------------------------------------------------------------------------------

# ---------------------------------------------------------------------------------------#
# --- Write a data frame with coefficients for each month and latitude band.----
...
# ---------------------------------------------------------------------------------------#
# --- Write a text file with SST latband1 algorithm coefficients ----
...
cat("# Algorithm version 6.3 PRELIMINARY SET 1\n",file=outfile, append=TRUE)
cat("# Model: bsst ~ ch11000 + btd.sst + secterm\n",file=outfile, append=TRUE)
...
# ---------------------------------------------------------------------------------------#
# --- NOW THAT COEFFICIENTS HAVE BEEN ESTIMATED COMPUTE LATBAND1 SSTs ----
# --- FOR ALL MATCHUPS.
...
x2 <- aa2$cen.11000 - aa2$cen.12000			# T45 = BT31 - BT32
x3 <- aa2$buoy.sst											# Buoy SST
x4 <- aa2$satz											    # Satellite zenith angle
x5 <- x2 * x3														# (BT31 - BT32) * buoy SST
x6 <- (secant.deg(x4) - 1) * x2         # Secant times (BT31 - BT32)

# --- Compute "latband1" SSTs and residuals for ALL the data.

SST.latband1.prelim <- coef1 + (coef2 * x1) + (coef3 * x5) + (coef4 * x6)
...
SST.latband1.prelim.res <- SST.latband1.prelim - aa2$buoy.sst		# SST residuals

# --- Add computed latband1 SST and residuals to data frame "aa2"

aa2 <- data.frame(aa2,
	SST.latband1.prelim = SST.latband1.prelim,
	SST.latband1.prelim.res = SST.latband1.prelim.res)
...
# ---------------------------------------------------------------------------------------#
# --- Statistics for preliminary SST latband1 residuals in TRAINING set
...
tt0 <- orig$SST.latband1.prelim.res
...
# --- Statistics for GAM residuals 

tt0 <- orig$SST.latband1.prelim.gam.res
